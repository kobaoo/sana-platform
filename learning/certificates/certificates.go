...
func s3Bucket() string {
	if secrets.S3Bucket != "" {
		return secrets.S3Bucket
	}
	return os.Getenv("S3Bucket")
}
...
func uploadToMinIO(ctx context.Context, id uuid.UUID, file io.Reader, size int64) (string, error) {
	...
	_, err := client.PutObject(ctx, bucket, key, file, size, minio.PutObjectOptions{
...
func handleUpload(w http.ResponseWriter, r *http.Request) {
	...
	key, err := uploadToMinIO(ctx, uid, file, -1)
	if err != nil {
