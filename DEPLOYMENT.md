...
**В локальной и dev-среде (`encore run`)** — миграции применяются **автоматически**. Encore при старте сам находит все `*.up.sql` в `db/migrations/` и накатывает те, которые ещё не применены. Ничего делать не нужно.

**В production (Kubernetes)** — поведение то же самое: Encore-образ при запуске Pod'а **автоматически применяет все pending миграции** перед тем как начать обслуживать запросы. Atlas CLI для применения миграций в production не нужен.

> Atlas CLI используется только разработчиками для **генерации** новых миграций при изменении схемы БД. Подробнее — в `MIGRATION.md`.
...
Перед первым запуском Pod'а убедиться что база данных `lms` **существует** и пользователь имеет нужные права (см. раздел 2). Сами таблицы Encore создаст автоматически.

### Проверка что миграции применились

После старта Pod'а проверить в логах — не должно быть ошибок вида `migration failed`. Для ручной проверки состояния через port-forward:
